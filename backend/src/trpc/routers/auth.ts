...
export const authRouter = router({
  login: publicProcedure.input(loginInputSchema).mutation(async ({ input, ctx }) => {
    const { accessToken, refreshToken } = await AuthServiceTRPC.login(input);

    ctx.setRefreshToken(refreshToken);
    return { accessToken, refreshTokenExpiry: refreshToken.expires };
  }),

  register: publicProcedure.input(registerInputSchema).mutation(async ({ input, ctx }) => {
    const { accessToken, refreshToken } = await AuthServiceTRPC.register(input);
  ...
  refresh: publicProcedure.mutation(async ({ ctx }) => {
    const { accessToken, refreshToken } = await AuthServiceTRPC.refresh(ctx.getRefreshToken());
